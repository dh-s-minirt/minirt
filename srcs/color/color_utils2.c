...
/*   Updated: 2023/01/02 14:49:13 by hyunkyu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	point_light_get(t_info_data *data)
{
	...
	dummy.ksn = 64;
	dummy.ks = 0.5;
	dummy.spec = powf(fmax(vec_dot(dummy.view_dir, dummy.reflect_dir), 0.0), dummy.ksn);
	dummy.specular = vec_mul((vec_mul(data->light.color, dummy.ks)), dummy.spec);
	dummy.brightness = data->light.ratio * LUMEN;
	return (vec_mul(vec_add(vec_add(dummy.diffuse, dummy.specular), data->am_light), dummy.brightness));
}
